...
export const socketChatAgent: AgentDefinition = registerAgent({
  ...
  // When images are attached, restrict to image-specific tools — same pattern
  // as http_chat. Keeps payload under OpenAI's 128-tool cap and keeps the
  // model focused on the visual task. handlers.ts passes payload.hasImages.
  tools: (args) => {
