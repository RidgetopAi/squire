 ...
 * (`tools: array too long. Expected maximum length 128`). Other
 * OpenAI-compatible providers have their own caps; 128 is the safe lower
 * bound. Truncates by insertion order so behavior is deterministic; loops
 * that need a tighter set should filter at the agent layer (see http_chat,
 * socket_chat). Logs the dropped names so it's visible when it fires.
 */
