...
/**
 * Batch reinforcement check for multiple memories.
 * More efficient than calling checkReinforcement individually.
 */
export async function checkReinforcementBatch(
...
/**
 * Manually promote a hypothesis memory to solid tier.
 * Used when user confirms a memory is correct.
 */
export async function promoteToSolid(memoryId: string): Promise<boolean> {
  try {
...
/**
 * Demote a solid memory back to hypothesis.
 * Used when user indicates a memory is incorrect or outdated.
 */
export async function demoteToHypothesis(
  memoryId: string,
