...
import {
  ...
  updateEventInGoogle,
  type GoogleEvent,
  type GoogleEventUpdateInput,
} from '../services/google/events.js';
...
interface CalendarEventTargetArgs {
...
function formatCalendarEventForTool(event: GoogleEvent & { calendar_name?: string }): Record<string, unknown> {
  return {
    id: event.id,
    title: event.summary,
    description: event.description,
    location: event.location,
    start_time: formatEventTime(event.start_time, event.all_day),
    end_time: formatEventTime(event.end_time, event.all_day),
    date_label: getDateLabel(event.start_time, event.all_day),
    time_local: getLocalTime(event.start_time, event.all_day),
    day_of_week: getDayOfWeek(event.start_time, event.all_day),
    all_day: event.all_day,
    status: event.status,
    calendar: event.calendar_name,
    google_calendar_id: event.google_calendar_id,
    google_event_id: event.event_id,
    is_recurring: !!event.rrule || !!event.recurring_event_id,
    html_link: event.html_link,
    updated_at: event.updated_at,
  };
}

function formatCalendarEventChoices(events: Array<GoogleEvent & { calendar_name?: string }>): Array<Record<string, unknown>> {
  return events.slice(0, 5).map(formatCalendarEventForTool);
}

function normalizeCalendarEventId(id: string): string {
  return id.startsWith('google-') ? id.replace('google-', '') : id;
}

async function resolveCalendarEventTarget(args: CalendarEventTargetArgs): Promise<
  | { ok: true; event: GoogleEvent & { calendar_name?: string } }
  | { ok: false; response: string }
> {
  const directId = args.calendar_event_id ?? args.event_id;
  if (directId) {
    const event = await getGoogleEventById(normalizeCalendarEventId(directId));
    if (!event || event.status === 'cancelled') {
      return {
        ok: false,
        response: JSON.stringify({
          error: `Calendar event with ID "${directId}" not found`,
          event: null,
        }),
      };
    }
    ...
    return {
      ok: true,
      event: { ...event, calendar_name: calendar?.summary ?? undefined },
    };
  }

  if (!args.event_title || args.event_title.trim().length === 0) {
    return {
      ok: false,
      response: JSON.stringify({
        error: 'Either calendar_event_id or event_title is required',
        event: null,
      }),
    };
  }
  ...
  const events = await getAllEvents({ timeMin: start, timeMax: end }) as Array<GoogleEvent & { calendar_name?: string }>;

  const exactMatches = events.filter((event) => (event.summary ?? '').toLowerCase() === title.toLowerCase());
  if (exactMatches.length === 1) return { ok: true, event: exactMatches[0]! };
  if (exactMatches.length > 1) {
    return {
      ok: false,
      response: JSON.stringify({
        error: `Multiple calendar events exactly match "${title}"`,
        ambiguous: true,
        choices: formatCalendarEventChoices(exactMatches),
      }),
    };
  }

  const partialMatches = events.filter((event) => (event.summary ?? '').toLowerCase().includes(title.toLowerCase()));
  if (partialMatches.length === 1) return { ok: true, event: partialMatches[0]! };
  if (partialMatches.length > 1) {
    return {
      ok: false,
      response: JSON.stringify({
        error: `Multiple calendar events match "${title}"`,
        ambiguous: true,
        choices: formatCalendarEventChoices(partialMatches),
      }),
    };
  }

  const words = title.toLowerCase().split(/\s+/).filter(Boolean);
  const wordMatches = events.filter((event) => {
    const haystack = `${event.summary ?? ''} ${event.description ?? ''} ${event.location ?? ''}`.toLowerCase();
    return words.length > 0 && words.every((word) => haystack.includes(word));
  });
  if (wordMatches.length === 1) return { ok: true, event: wordMatches[0]! };
  if (wordMatches.length > 1) {
    return {
      ok: false,
      response: JSON.stringify({
        error: `Multiple calendar events may match "${title}"`,
        ambiguous: true,
        choices: formatCalendarEventChoices(wordMatches),
      }),
    };
  }
  ...
  return {
    ok: false,
    response: JSON.stringify({
      error: `No upcoming calendar event found matching "${title}"`,
      event: null,
      suggestion: 'Use get_upcoming_events or get_todays_events to find the exact event ID.',
    }),
  };
}

async function getWritableCalendarForEvent(event: GoogleEvent): Promise<
  | { ok: true; calendar: NonNullable<Awaited<ReturnType<typeof getCalendar>>> }
  | { ok: false; response: string }
> {
  const calendar = await getCalendar(event.google_calendar_id);
  if (!calendar) {
    return {
      ok: false,
      response: JSON.stringify({ error: 'Calendar not found for event', event: null }),
    };
  }
  if (calendar.sync_direction === 'read_only') {
    return {
      ok: false,
      response: JSON.stringify({
        error: `Calendar "${calendar.summary}" is read-only. Change sync direction in settings to enable editing.`,
        event: formatCalendarEventForTool({ ...event, calendar_name: calendar.summary ?? undefined }),
      }),
    };
  }
...
interface UpdateCalendarEventArgs extends CalendarEventTargetArgs {
  title?: string | null;
...
async function handleUpdateCalendarEvent(args: UpdateCalendarEventArgs): Promise<string> {
  try {
    const resolved = await resolveCalendarEventTarget(args);
    if (!resolved.ok) return resolved.response;

    const calendarResult = await getWritableCalendarForEvent(resolved.event);
    if (!calendarResult.ok) return calendarResult.response;

    const updates: GoogleEventUpdateInput = {};
    ...
    if (args.location !== undefined) {
      updates.location = args.location?.trim() || null;
      changedFields.push('location');
    }
    if (args.start_time !== undefined) {
      updates.due_at = parseDateTime(args.start_time, args.all_day ?? resolved.event.all_day);
      changedFields.push('start_time');
    }
    if (args.duration_minutes !== undefined) {
      if (args.duration_minutes <= 0) {
        return JSON.stringify({ error: 'duration_minutes must be positive', event: null });
      }
    ...
    if (changedFields.length === 0) {
      return JSON.stringify({
        error: 'At least one calendar event field is required',
        event: null,
      });
    }

    await updateEventInGoogle(calendarResult.calendar, resolved.event.event_id, updates);
    const updatedEvent = await getGoogleEventById(resolved.event.id);

    return JSON.stringify({
      message: `Calendar event "${updatedEvent?.summary ?? resolved.event.summary ?? 'Untitled event'}" updated`,
      event: updatedEvent
        ? formatCalendarEventForTool({ ...updatedEvent, calendar_name: calendarResult.calendar.summary ?? undefined })
        : formatCalendarEventForTool({ ...resolved.event, calendar_name: calendarResult.calendar.summary ?? undefined }),
      changed_fields: [...new Set(changedFields)],
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return JSON.stringify({ error: `Failed to update calendar event: ${message}`, event: null });
  }
}

async function handleDeleteCalendarEvent(args: CalendarEventTargetArgs): Promise<string> {
  try {
    const resolved = await resolveCalendarEventTarget(args);
    if (!resolved.ok) return resolved.response;
    ...
    await deleteEventInGoogle(calendarResult.calendar, resolved.event.event_id);

    return JSON.stringify({
      message: `Calendar event "${resolved.event.summary ?? 'Untitled event'}" deleted`,
      deleted: true,
      event: {
        id: resolved.event.id,
        title: resolved.event.summary,
        calendar: calendarResult.calendar.summary,
      },
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return JSON.stringify({ error: `Failed to delete calendar event: ${message}`, event: null });
  }
}
...
export const tools: ToolSpec[] = [
  ...
  },
  {
    ...
    description:
      'Edit an existing writable Google Calendar event by ID or fuzzy title. Use for requests like "move that meeting to 3", "shorten my dentist appointment", "change the location", or "rename that calendar event". Returns ambiguity choices when matching is uncertain.',
    parameters: {
      type: 'object',
      properties: {
        calendar_event_id: {
          type: 'string',
          description: 'The cached calendar event UUID from get_upcoming_events/get_todays_events. A google-{id} value is also accepted.',
        },
        event_id: {
          type: 'string',
          description: 'Alias for calendar_event_id.',
        },
        event_title: {
          type: 'string',
          description: 'Current event title/name to find. Supports exact, partial, and word-based matching across upcoming events.',
        },
        title: {
          type: ['string', 'null'],
          description: 'New event title, or null to clear it.',
        },
        description: {
          type: ['string', 'null'],
          description: 'New event description, or null to clear it.',
        },
        location: {
          type: ['string', 'null'],
          description: 'New event location, or null to clear it.',
        },
        start_time: {
          type: 'string',
          description: 'New start date/time in ISO 8601 format. For all-day events, use YYYY-MM-DD.',
        },
        duration_minutes: {
          type: 'number',
        ...
        timezone: {
          type: 'string',
          description: 'Timezone for the event, usually the configured user timezone.',
        },
      },
      required: [],
    },
    handler: handleUpdateCalendarEvent as ToolHandler,
  },
  {
    name: 'delete_calendar_event',
    description:
      'Delete/cancel an existing writable Google Calendar event by ID or fuzzy title. Use when the user clearly asks to cancel, delete, or remove an event from their calendar.',
    parameters: {
      type: 'object',
      properties: {
        calendar_event_id: {
    ...
    handler: handleDeleteCalendarEvent as ToolHandler,
  },
];
